...
public class PairedAttributeRating : MonoBehaviour
{
    ...
    public Slider[] AAttributes;

    public Slider[] BAttributes;
    ...
    private void OnNextPairButton()
    {
        if (currentScene.name != "_config")
        {
            ...
            {
                AAttributes[i].value = 0;
            }
            for (int i = 0; i <= BAttributes.Length; i++)
            {
                AAttributes[i].value = 0;
            }
        }
